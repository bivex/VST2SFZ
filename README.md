...
* **`render_melody.py`** — скрипт для оффлайн-рендеринга MIDI-мелодии (Бах) напрямую через VST-движок.
* **`render_sfz_melody.py`** — скрипт для воспроизведения той же мелодии с использованием только отрендеренных WAV-сэмплов и SFZ-карты (без участия VST).
* **`render_sfz_gymnopedie.py`** — сэмплерный рендеринг пьесы Эрика Сати «Гимнопедия № 1».
* **`render_sfz_moonlight.py`** — сэмплерный рендеринг первой части «Лунной сонаты» Бетховена в качестве 24-bit / 96 kHz.
...
Все три скрипта рендеринга автоматически генерируют красивый стерео-звук в качестве **24-bit / 96 kHz** и сохраняют его в корневой каталог проекта.
...
```bash
python render_sfz_moonlight.py
...
```bash
python render_sfz_gymnopedie.py
