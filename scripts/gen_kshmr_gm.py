...
Fills all 128 programs by distributing samples across GM program families.
"""
...
OUT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "sfz", "KSHMR_Vol5_128GM.sfz")
...
def classify(path):
    ...
    if "bass" in p and "drum loop" not in p:
        return "bass", "any"
    ...
    if "drum loop" in p or "drum pattern" in p:
        return "drumloop", "loop"

    if "action loop" in p:
        return "actionloop", "loop"
    ...
    if "songstarter" in p or "ss -" in p:
        return "songstarter", "full"
...
def assign_program(fam, sub, path):
    ...
    if fam == "synth":
        if "pluck" in p:
            return 88 + (hash(path) % 7)  # 88-94 (pad 1-7)
        if "stab" in p:
            if "brass" in p:
                return 61
            return 80 + (hash(path) % 14)  # 80-93 (lead 1-7, pad 1-6)
        if "screech" in p:
            return 102
        if "lead" in p or "melody" in p:
            return 80 + (hash(path) % 8)
        if "chord" in p:
            return 89 + (hash(path) % 7)  # 89-95
        if "arp" in p:
    ...
    if "bell" in p or "tink" in p or "agogo" in p:
        return 112  # Tinkle Bell
    if "cowbell" in p or "wood" in p:
        return 115  # Woodblock
    if "taiko" in p or "orchestral drum" in p or "orc drum" in p:
        return 116  # Taiko
    if "steel" in p or "steeldrum" in p:
        return 114  # Steel Drums
    if "melodic tom" in p:
        return 117  # Melodic Tom
    if "synth drum" in p:
        return 118  # Synth Drum
    if "reverse cymbal" in p or "reverse cym" in p:
        return 119  # Reverse Cymbal
    ...
    if "fret" in p or "guitar noise" in p:
        return 120
    if "bell" in p or "chime" in p:
        return 112
    if "taiko" in p or "orc" in p:
        return 116
    if "damage" in p or "hit" in p or "impact" in p:
        return 127
...
# ============================================================
# Fill missing programs by borrowing from related families
# ============================================================
...
def fill_missing(gm_map):
    ...
    # For each missing program, pick a representative from a related program
    for prog in missing:
        # Find related programs that have samples
        candidates = []
        if prog in list(range(8, 16)):  # Chromatic Perc
            # Look for bell/chime/pluck sounds from drums or synth
            for p in [112, 113, 114, 115, 116, 117, 118, 119, 56, 57, 88, 89]:
                if gm_map[p]:
                    candidates.extend(gm_map[p])
            if not candidates:
                candidates = gm_map[0][:5]  # drum samples
        elif prog in list(range(16, 24)):  # Organ family
            candidates = gm_map[21][:3]  # accordion
            if not candidates:
                candidates = gm_map[16][:3]  # organ
            if not candidates:
                candidates = gm_map[46][:3]  # harp
        elif prog in list(range(96, 102)) or prog == 103:  # FX
            candidates = (
                gm_map[80][:5]
                + gm_map[81][:5]
                + gm_map[88][:3]
            ...
            candidates = gm_map[0][-20:] + gm_map[47][:5]  # drums + timpani
        elif prog == 127:  # Gunshot
            candidates = gm_map[120][:5] + gm_map[121][:5] + gm_map[55][:3]
        elif prog == 48:  # String Ensemble
            candidates = gm_map[40][:10] + gm_map[41][:5]
        elif prog == 93 or prog == 94 or prog == 95:  # Pad 6-8
            candidates = gm_map[88][:10] + gm_map[89][:10] + gm_map[90][:5]
        elif prog == 106:  # Shamisen
            candidates = gm_map[104][:5] + gm_map[25][:5] + gm_map[27][:5]
            ...
            # Generic fallback: pick from nearby program
            nearby = [prog - 1, prog + 1, prog - 8, prog + 8]
            for p in nearby:
        ...
        if candidates:
            # Add first 3 candidates to the missing program
...
def gen_sfz(gm_map):
    lines = []
    lines.append("// KSHMR Vol.5 Complete Edition - 128 General MIDI")
    ...
    if missing:
        lines.append(f"// Programs without dedicated samples: {missing}")
    lines.append("")
...
def main():
    ...
    print(f"Found {len(all_files)} usable files")
