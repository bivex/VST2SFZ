...
Post-processing: transparent EQ per instrument group + LUFS normalisation,
...
def get_group(slot: int) -> str:
    for group, r in INSTRUMENT_GROUPS.items():
        if slot in r:
            return group
    return "default"
...
def write_drum_section(f, drum_samples_dir):
    ...
    for note, name in DRUM_NOTES:
        ...
        f.write(f"// {note} — {name}\n")
        f.write(
            f"<region> sample={p64}  lokey={note} hikey={note} lovel=0   hivel=80\n"
        )
        f.write(
            f"<region> sample={p127} lokey={note} hikey={note} lovel=81  hivel=127\n"
        )
        written += 1
...
def main():
    ...
    for idx, (slot, (syx_path, voice_slot)) in enumerate(sorted(mapping.items())):
        ...
        print(
            f"[{idx + 1}/{total}] slot {slot:02d}  {inst_name}  ({syx_basename} v{voice_slot})"
        )
