...
copy the chosen samples into General_MIDI_samples_drums/, and append the
corresponding regions to General_MIDI_sfizz_drums.sfz.

Each GM note gets a single best-match KSHMR sample (one-shot, dry, neutral
variant preferred). One velocity layer (v127) is copied; v064 uses the same
audio so the kit keeps its dual-velocity-region convention.
"""
...
# Choices prioritise one-shots (Hit/Closed/Clean/Classic) over loops/FX,
# and pick the most neutral/canonical variant so the sound matches the
# standard GM percussion name as closely as the library allows.
GM_TO_KSHMR = {
    39: "Drums/Claps/Main/KSHMR Main Clap - Mid - Snap.wav",            # Hand Clap
    52: "Drums/Cymbals/Crashes/KSHMR Crash - Acoustic - Classic Kit.wav", # Chinese Cymbal (bright crash substitute)
...
def main():
    # 1. Verify all candidates exist before touching anything
    ...
    # 3. Rebuild SFZ by inserting new regions in note order
    #    Parse existing SFZ: header (// + <group> + ampeg), then sorted regions.
    print("\n=== Rebuilding SFZ ===")
    ...
    # Build a sorted map note -> (v064_sample, v127_sample) from existing + new
    region_map = {}
    for line in regions:
        m_lo = re.search(r"lokey=(\d+)", line)
        m_s = re.search(r"sample=([^ ]+\.wav)", line)
        m_v = re.search(r"hivel=(\d+)", line)
        if m_lo and m_s and m_v:
            note = int(m_lo.group(1))
            hivel = int(m_v.group(1))
            sample = os.path.basename(m_s.group(1))
            region_map.setdefault(note, {})[hivel] = sample

    # Emit sorted SFZ
    out = []
    out.extend(header if header else ["// GM Drum Kit — sfizz (KSHMR Vol.5)\n<group>\nampeg_attack=0.001 ampeg_release=0.05\n\n"])
    for note in sorted(region_map):
        layers = region_map[note]
        v064 = layers.get(80)
        v127 = layers.get(127)
        if v064:
            out.append(f"<region> sample={DRUM_DIR}/{v064} lokey={note} hikey={note} lovel=0 hivel=80\n")
        if v127:
            out.append(f"<region> sample={DRUM_DIR}/{v127} lokey={note} hikey={note} lovel=81 hivel=127\n")

    with open(SFZ_PATH, "w") as f:
        f.writelines(out)
    print(f"  Wrote {len(region_map)} drum notes ({2*len(region_map)} regions) to {os.path.basename(SFZ_PATH)}.")
